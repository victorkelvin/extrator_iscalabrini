...
@shared_task
def bahia(filepath):
    pattern = r"([\w\s\u0080-\uFFFF]+), proc. (\d+\.\d+\.\d+\.\d+-\d+|\d+),.*?(\d{8}),.*?\$?(\d+\.?\d+,\d+)" # Quase 24h pra fazer 1 linha de código --', pqp!
    ...
    try:
        with open(filepath, "rb") as file:
            ...
            # diario_dict = [diario.toDict() for diario in diarios]  #type: ignore
            if diarios:
